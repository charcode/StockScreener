...
public class YahooWebDataBalanceSheetDao extends AbstractYahooFinanceWebParser<BalanceSheet> implements BalanceSheetDao {
    ...
    @Override
    public SortedMap<Date, BalanceSheet> getBalanceSheetForSymbol(String symbol, boolean annual) {
	SortedMap<Date, BalanceSheet> balanceSheetsOnPage = getStatementsForSymbol(symbol, annual);
	return balanceSheetsOnPage;
    }

    @Override
    public Map<String, SortedMap<Date, BalanceSheet>> getBalanceSheetsBySymbol( Set<String> symbols, boolean annual) {
	...
	if (symbols != null) {
		...
		SortedMap<Date, BalanceSheet> balanceSheetForSymbol = getBalanceSheetForSymbol(symbol, annual);
	...
	BalanceSheet bs = new BalanceSheet(symbol,
		values.get(CASH_AND_CASH_EQUIVALENTS).get(i),// cashAndEquivalent,
		...
		values.get(LONG_TERM_INVESTMENTS).get(i),// longTermInvestments,
		values.get(PROPERTY_PLANT_AND_EQUIPMENT).get(i),// propertyPlantAndEquivalent,
		values.get(GOODWILL).get(i),// goodwill,
		values.get(INTANGIBLE_ASSETS).get(i),// intangibleAssets,
		values.get(ACCUMULATED_AMORTIZATION).get(i),// accumulatedAmortization,
		values.get(OTHER_ASSETS).get(i),// otherAssets,
		values.get(DEFERRED_LONG_TERM_ASSET_CHARGES).get(i),// deferredLongTermAssetCharges,
		values.get(ACCOUNTS_PAYABLE).get(i),// accountPayable,
		values.get(SHORT_CURRENT_LONG_TERM_DEBT).get(i),// shortCurrentLongTermDebt,
		values.get(OTHER_CURRENT_LIABILITIES).get(i),// otherCurrentLiabilities,
		values.get(LONG_TERM_DEBT).get(i),// longTermDebt,
		values.get(OTHER_LIABILITIES).get(i),// otherLiabilities,
		values.get(DEFERRED_LONG_TERM_LIABILITY_CHARGES).get(i),// deferredLongTermLiabilityCharges,
		values.get(MINORITY_INTEREST).get(i),// minorityInterest,
		values.get(NEGATIVE_GOODWILL).get(i),// negativeGoodwill,
		values.get(MISC_STOCKS_OPTIONS_WARRANTS).get(i),// miscStocksOptionsWarrants,
		values.get(REDEEMABLE_PREFERRED_STOCK).get(i),// redeemablePreferredStocks,
		values.get(COMMON_STOCK).get(i),// commonStock,
		values.get(PREFERRED_STOCK).get(i),// preferredStock,
		values.get(RETAINED_EARNINGS).get(i),// retainedEarnings,
		values.get(TREASURY_STOCK).get(i),// treasuryStock,
		values.get(CAPITAL_SURPLUS).get(i),// capitalSurplus,
