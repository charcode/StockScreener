...
public class YahooDataConverterImpl implements YahooDataConverter {
	...
	private Stock createStock(yahoofinance.Stock s) {
		...
		Stock ret = new Stock(name, symbol, symbol, name, stockExchange,
				currency);
		return ret;
	}
	...
	private Economic extractTodayEconomic(yahoofinance.Stock yahooStock) {
		...
		if (yahooStock != null) {
			...
			if (dividend != null) {
				...
				annualDividendYieldPercent = getDoubleValue(dividend
			...
			ret = new Economic(lastTradeTime, bid, bidSize, ask, askSize,
					...
					sharesOutstanding, sharesOwned, annualDividendYield,
					annualDividendYieldPercent, dividendExDate, dividendPayDate);
		}
		return ret;
	}
