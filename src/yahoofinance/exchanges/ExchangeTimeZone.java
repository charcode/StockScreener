...
public class ExchangeTimeZone {
    ...
    public ExchangeTimeZone(Logger logger) {
    	this.logger = logger;
    }
    ...
    public  TimeZone get(String suffix) {
        if(SUFFIX_TIMEZONES.containsKey(suffix)) {
            return SUFFIX_TIMEZONES.get(suffix);
        }
        logger.warn("Cannot find time zone for exchange suffix: '" + suffix + "'. Using default: America/New_York");
     ...
     */
    public  TimeZone getStockTimeZone(String symbol) {
        ...
        return get(split[split.length - 1]);
    }
}
