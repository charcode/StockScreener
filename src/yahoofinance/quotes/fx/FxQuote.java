...
public class FxQuote {
    ...
    public FxQuote(String symbol, Logger logger) {
    	this(symbol,BigDecimal.ZERO, logger);
    }
    
    public FxQuote(String symbol, BigDecimal price, Logger logger) {
        this.symbol = symbol;
        this.price = price;
        this.logger = logger;
    }
    ...
    public BigDecimal getPrice(boolean refresh) throws IOException {
        if(refresh) {
            FxQuotesRequest request = new FxQuotesRequest(this.symbol, logger);
