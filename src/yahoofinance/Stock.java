...
public class Stock {
    ...
    public Stock(String symbol, Logger logger) {
        this.symbol = symbol;
        this.logger = logger;
    }
    ...
    private void update() throws IOException {
        StockQuotesRequest request = new StockQuotesRequest(this.symbol, logger);
        ...
        if(data != null) {
            ...
            logger.info("Updated Stock with symbol: {0}", this.symbol);
        } else {
            logger.error("Failed to update Stock with symbol: "+ this.symbol);
        }
    }
    ...
    public List<HistoricalQuote> getHistory(Calendar from, Calendar to, Interval interval) throws IOException {
        HistQuotesRequest hist = new HistQuotesRequest(this.symbol, from, to, interval, logger);
    ...
    public void print() {
        ...
        for (Field f : this.getClass().getDeclaredFields()) {
            try {
                ...
                LogManager.getLogger(Stock.class).error("illegal argument",ex);
            } catch (IllegalAccessException ex) {
            	LogManager.getLogger(Stock.class).error("Illegal access",ex);
            }
        }
        System.out.println("--------------------------------");
    }
